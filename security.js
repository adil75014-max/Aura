...
var _SUPA_HOST = (typeof SUPABASE_URL !== "undefined" && SUPABASE_URL)
    ? SUPABASE_URL.replace(/^https?:\/\//, "")
    : ((typeof window !== "undefined" && window.SUPABASE_URL) ? window.SUPABASE_URL.replace(/^https?:\/\//, "") : "supabase.co");
var ALLOWED_OUTBOUND = [
    _SUPA_HOST, "api.meteo-concept.com",
...
(function checkNetworkAnomaly() {
    ...
    var start = Date.now();
    var _supaBase = (typeof SUPABASE_URL !== "undefined") ? SUPABASE_URL : (window.SUPABASE_URL || "");
    var _supaKey  = (typeof SUPABASE_KEY !== "undefined") ? SUPABASE_KEY  : (window.SUPABASE_KEY  || "");
    if (!_supaBase || !_supaKey) return;   // config pas encore prête → on saute
    fetch(_supaBase + "/rest/v1/?limit=0", {
        method: "HEAD",
        headers: { "apikey": _supaKey }
    }).then(function() {
